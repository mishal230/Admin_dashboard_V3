...
import { Search, Truck, Phone, Link as LinkIcon, CheckCircle, XCircle, Plus, Edit, Trash2 } from 'lucide-react'
...
const fetchCouriers = async () => {
  try {
    ...
    const { data, error } = await supabase
      .from('couriers') // ✅ correct table name
      .select('*')
      .order('courier_id', { ascending: true })

    if (error) throw error

    console.log('Couriers:', data) // ✅ debug log
    setCouriers(data || [])
  } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCourier) {
        ...
        const { error } = await supabase.from('couriers').insert([formData])
        if (error) throw error
      }

      setFormData({ name: '', contact_number: '', api_endpoint: '', status: 'active' })
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {couriers.map((courier) => (
                  <tr key={courier.courier_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <LinkIcon className="w-4 h-4 mr-2 text-gray-400 inline" />
                      {courier.api_endpoint || 'N/A'}
                    </td>
