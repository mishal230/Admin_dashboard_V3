...
export default function Customers() {
 ...
 const fetchCustomers = async () => {
  try {
    ...
    // 2. Get addresses separately (in case no FK relationship exists)
    const { data: addressesData, error: addressError } = await supabase
      .from('addresses')
      .select('*')

    if (addressError) {
      console.warn('No addresses table or relation found:', addressError.message)
    }

    // 3. Merge addresses into customers manually
    const customersWithAddresses = (customersData || []).map((customer) => {
    ...
    // 4. Attach order stats
    const customersWithStats = await Promise.all(
      customersWithAddresses.map(async (customer) => {
